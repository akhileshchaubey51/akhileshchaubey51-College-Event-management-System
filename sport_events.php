    ...
    <div id="page1">
        ...
        <div class="element-container">
            <?php foreach ($sportEvents as $event): ?>
                <div class="filterelement" data-event-type="<?php echo htmlspecialchars($event['event_name']); ?>">
                    <div class="event-image" style="background-image: url('<?php echo isset($event['image_path']) && !empty($event['image_path']) ? htmlspecialchars($event['image_path']) : 'default-image.jpg'; ?>');"></div>
                    <p><?php echo htmlspecialchars($event['event_name']); ?></p>
                    <span><?php echo nl2br(htmlspecialchars($event['category'])); ?></span>
                    <?php if ($isLoggedIn): ?>
    ...
    <script>
        ...
        document.querySelectorAll('.buttons').forEach(button => {
            button.addEventListener('click', function() {
                ...
                // Show or hide filter elements based on the selected filter
                document.querySelectorAll('.filterelement').forEach(element => {
                    const eventType = element.getAttribute('data-event-type');
                    console.log('Event Type:', eventType); // Debugging: Check event type
