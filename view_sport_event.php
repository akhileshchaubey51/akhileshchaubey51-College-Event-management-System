...
$stmt = $conn->prepare("SELECT * FROM sport_events WHERE id = ?");
...
<html>
<head>
    <title><?= htmlspecialchars($event['event_name']) ?> - Sport Event</title>
    <style>
        body { font-family: 'Times New Roman'; padding: 30px; background: #f9f9f9; }
        .event-container {
        ...
        .btn {
            display: inline-block; margin-top: 20px; background: #28a745; color: white;
...
<body>
    ...
    <p><strong>Description:</strong><br><?php echo nl2br(htmlspecialchars($event['description'])); ?></p>
        <a class="btn" href="register_event.php?event_id=<?= $event['id'] ?>&type=Sport">Register Now</a>
    </div>
</body>
</html>
